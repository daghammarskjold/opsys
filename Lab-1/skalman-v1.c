...
void handle_command(char words[][MAX_WORD_LENGTH + 1], int nr_words) {
    if (strcmp(words[0], "exit") == 0) {
        ...
        if (nr_words < 2) //has to be more than two arguments
           ...
           char *temp[nr_words];
            //build the temp array depending on the amout of arguments
            ...
            int childpid = fork(); //starting child process here
            printf("skalman: forked\n");
            if (childpid == 0){
                if (strcmp(temp[0], "exit") == 0){ //if we wanted to exit immedietly
                    exit(EXIT_SUCCESS);
                }
                execvp(temp[0], temp);
